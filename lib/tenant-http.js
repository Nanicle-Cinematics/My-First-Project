...
const { authenticate, resolveSessionUser, createSession, createPendingMfaSession, destroySession } = require('./auth');
...
function createTenantApp({ sessionSecret, pool, sessionTableName = 'session' } = {}) {
  ...
  app.use((req, res, next) => {
    ...
    if (!user?.church?.suspendedAt) return next();
    ...
    const message = user.church.suspensionReason
    ...
    if (req.path.startsWith('/api/') || req.is('application/json')) {
      return res.status(403).json({ error: 'church suspended', message });
    }
