...
// Compute a short content hash of public/styles.css at module load.
// This becomes the cache-buster on every <link rel="stylesheet"> tag so
// the moment a deploy ships new CSS, every browser fetches the fresh copy
// instead of serving a stale cached version.
let STYLES_VER = 'dev';
try {
  const css = fs.readFileSync(path.join(__dirname, '..', 'public', 'styles.css'));
  STYLES_VER = crypto.createHash('sha1').update(css).digest('hex').slice(0, 10);
} catch (_) {}
