...
// Compute a short content hash of each stylesheet at module load. This
// becomes the cache-buster on its <link rel="stylesheet"> tag so the moment a
// deploy ships new CSS, every browser fetches the fresh copy instead of
...
let STYLES_VER = cssVersion('styles.css');
// The iOS-26 glass theme rides on top of styles.css as its own file rather
// than a tenth layer appended inside it — see the header of
// public/theme-ios26.css for why. It must load second; it wins on order.
let THEME_VER = cssVersion('theme-ios26.css');

module.exports.createShell = function createShell(deps) {
  const { CHURCH_NAME, NAV, esc, initials, flashHtml, scriptureOfDay, listBackups, deleteAllScopes = {} } = deps;
  const STYLES_HREF = `/static/styles.css?v=${STYLES_VER}`;
  const THEME_HREF = `/static/theme-ios26.css?v=${THEME_VER}`;
  const STYLE_TAGS = `<link rel="stylesheet" href="${STYLES_HREF}">
<link rel="stylesheet" href="${THEME_HREF}">`;
  // Anti-flicker theme paint: applied inline in <head>, before first paint,
  // so the page never flashes the wrong theme. Shared by both templates
  // below instead of being duplicated verbatim — a prior version of this
  // file had two independent copies that silently diverged when only one
  // was updated.
  const THEME_INIT_SCRIPT = `<script>(function(){try{var saved=localStorage.getItem('theme');var mq=window.matchMedia&&matchMedia('(prefers-color-scheme:dark)');document.documentElement.setAttribute('data-theme',saved||(mq&&mq.matches?'dark':'light'));}catch(e){}})();</script>`;
  // The main app shell's body script (below) attaches its own system-preference
  // listener alongside the theme-toggle button, so it doesn't need this. The
  // bare/auth shell has no toggle button and no other script, so without this
  // it would never notice an OS theme change made while the tab stays open.
  const THEME_SYSTEM_SYNC_SCRIPT = `
<script>
(function () {
  try {
...
(function () {
  function enhancePasswordFields() {
    document.querySelectorAll('input[type="password"], input[data-password-toggle="1"]').forEach(function (input) {
  ...
  function layout({ title, subtitle, subtitleHtml, body, active, flash, flashType, user, bare, noHeader, churchName }) {
    const displayChurchName = churchName || CHURCH_NAME;
    if (bare) {
      return `<!doctype html>
<html lang="en">
<head>
...
<title>${esc(title)} · ${esc(displayChurchName)}</title>
${THEME_INIT_SCRIPT}
${STYLE_TAGS}
</head>
<body>
<div class="auth-shell">
  <div class="auth-layout">
    <section class="auth-intro landing-intro" aria-label="Church management system overview">
      <div class="landing-brand-block">
        <img class="auth-logo" src="/branding/logo" alt="Church logo">
        <div>
          <div class="auth-kicker">Refined Ministry Operations</div>
          <h2>${esc(displayChurchName)}</h2>
          <p>Manage members, attendance, giving, services, communications and reports from one secure place.</p>
        </div>
      </div>
      <div class="landing-feature-grid" aria-label="Core modules">
        <div class="landing-feature">
          <span>👥</span>
          <strong>Members</strong>
          <small>Profiles, day-born groups and Bible classes.</small>
        </div>
        <div class="landing-feature">
          <span>₵</span>
          <strong>Finance</strong>
          <small>Offerings, expenses, harvests and reports.</small>
        </div>
        <div class="landing-feature">
          <span>✓</span>
          <strong>Attendance</strong>
          <small>Events, check-ins and follow-up lists.</small>
        </div>
      </div>
      <div class="landing-status-row" aria-label="System assurances">
        <span><b></b> Secure access</span>
        <span><b></b> Cloud hosted</span>
        <span><b></b> Backup ready</span>
      </div>
    </section>
    <div class="auth-card">
      <div class="brand-mini">
        <img class="brand-mini-logo" src="/branding/logo" alt="">
        ${esc(displayChurchName)}
      </div>
      <h1>${esc(title)}</h1>
...
${THEME_SYSTEM_SYNC_SCRIPT}
    ...
    const navSections = [
      ...
      { label: 'Services / Events', hrefs: ['/events', '/preaching', '/communications'] },
      { label: 'Reports', hrefs: ['/reports', '/operations', '/tenant', '/backups', '/errors', '/security/audit'] },
      { label: 'Settings', hrefs: ['/users', '/settings', '/help'] },
    ];
    ...
    const deleteScope = user && user.canPermanentDeleteAll
      ? Object.entries(deleteAllScopes).find(([, scope]) => (scope.active || []).includes(active))
      : null;
    const deleteAllAction = deleteScope
      ? `<a class="delete-all-action" href="/delete-all/${encodeURIComponent(deleteScope[0])}" title="Permanently delete all ${esc(deleteScope[1].label)} data">Delete all</a>`
      : '';
    const backup = (() => {
      try {
...
<div class="app">
  <aside class="sidebar" id="app-nav">
    <div class="brand">
      <img class="brand-logo-img" src="/branding/logo" alt="" aria-hidden="true">
      <div>
        <div class="name">${esc(displayChurchName)}</div>
  ...
  <div class="main">
    ...
    <main class="page" id="main" tabindex="-1">
      ...
      <script>
        ...
        // Persistent light/dark toggle, with system preference as the default.
        (function () {
        ...
        (function () {
          document.querySelectorAll('[data-card-href]').forEach(function (card) {
            ...
            card.addEventListener('click', function (e) {
              ...
              if (e.target.closest('[data-card-href]') !== card) return;
              go();
            });
        ...
        // A table can override how many leading columns count as "key" via
        // data-key-cols (see lib/views.js's table() helper) — the default of
        // 2 is wrong for tables where the decision-relevant fields (e.g.
        // amount, status) aren't in the first two columns.
        (function () {
          var DEFAULT_KEY_COLS = 2;
          var mq = window.matchMedia('(max-width: 640px)');
          function enhance() {
            if (!mq.matches) return;
            document.querySelectorAll('table.data-table:not(.members-table)').forEach(function (table) {
              var keyCols = Number(table.dataset.keyCols) || DEFAULT_KEY_COLS;
              table.querySelectorAll('tbody tr').forEach(function (tr) {
                if (tr.dataset.rEnhanced) return;
                var tds = tr.querySelectorAll('td');
                if (tds.length <= keyCols) { tr.dataset.rEnhanced = '1'; return; }
                var hasExtra = false;
                for (var i = keyCols; i < tds.length; i++) {
    ...
    <footer class="footer">
      <div>© ${new Date().getFullYear()} ${esc(displayChurchName)}. All rights reserved.</div>
      <div class="status">
