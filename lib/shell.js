...
(function () {
  ...
  const FINANCE_SUBNAV_GROUPS = [
    { label: null, links: [['/finance', 'Overview']] },
    { label: 'Income', links: [
  ...
  function financeSubNav(activePath, escFn) {
    const groups = FINANCE_SUBNAV_GROUPS.map((group) => {
      const label = group.label ? `<div class="subnav-label">${escFn(group.label)}</div>` : '';
      const links = group.links.map(([href, text]) =>
        `<a class="${href === activePath ? 'active' : ''}" href="${href}">${escFn(text)}</a>`
      ).join('');
      return `<div class="subnav-group">${label}${links}</div>`;
    }).join('');
    return `<div class="finance-subnav">${groups}</div>`;
  }
  ...
  <div class="auth-layout">
    <section class="auth-intro landing-intro" aria-label="Church management system overview">
      <div class="landing-brand-block">
        <img class="auth-logo" src="/static/logo.png" alt="The Methodist Church Ghana">
        <div>
    ...
    <div class="auth-card">
      <div class="brand-mini">
        <img class="brand-mini-logo" src="/static/logo.png" alt="">
        ${esc(CHURCH_NAME)}
      </div>
      <h1>${esc(title)}</h1>
...
${PASSWORD_TOGGLE_SCRIPT}
    ...
    const isFinancePath = active && (active === '/finance' || active.startsWith('/finance/'));
    ...
    const navHtml = navSections.map((section) => {
      ...
      return `<div class="nav-section">
        ...
        ${links.map(([href, label, icon]) => {
          const isFinanceLink = href === '/finance';
          const cls = isFinanceLink && isFinancePath
            ? (active === '/finance' ? 'active' : 'active-parent')
            : (href === active ? 'active' : '');
          const subNav = isFinanceLink && isFinancePath ? financeSubNav(active, esc) : '';
          return `<a class="${cls}" href="${href}"><span class="ico">${icon}</span><span class="nav-text">${esc(label)}</span></a>${subNav}`;
        }).join('')}
      </div>`;
    }).join('');
...
<div class="app">
  <aside class="sidebar" id="app-nav">
    <div class="brand">
      <img class="brand-logo-img" src="/static/logo.png" alt="" aria-hidden="true">
      <div>
  ...
  <div class="main">
    ...
    <main class="page" id="main" tabindex="-1">
      ...
      <script>
        ...
        // Theme follows the operating system appearance preference.
        (function () {
          var root = document.documentElement;
          var btn = document.querySelector('.theme-toggle');
          var mq = window.matchMedia && matchMedia('(prefers-color-scheme:dark)');
          function apply() {
            var t = mq && mq.matches ? 'dark' : 'light';
            root.setAttribute('data-theme', t);
            if (btn) {
              btn.textContent = t === 'dark' ? '☀️' : '🌙';
              btn.title = 'Appearance follows system settings';
              btn.setAttribute('aria-label', 'Appearance follows system settings');
            }
          }
