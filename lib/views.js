...
// Page title block. Renders as a large inline title rather than the coloured
// banner it used to be — see the header of public/theme-ios26.css.
//
// The "Command Center" kicker that sat above every title is gone: it labelled
// thirteen unrelated pages identically, said nothing about any of them, and
// read as software marketing inside a tool for running a church. The class
// names stay so the stylesheet keeps one place to target.
function pageHero(title, subtitle) {
  return `<div class="page-hero command-center" data-command-center="true"><div class="hero-text">
...
// opts.keyCols overrides how many leading columns the mobile row-collapse
// script (lib/shell.js) shows before "Show details" — the default of 2 is
// wrong for tables where the decision-relevant fields (e.g. amount, status)
// aren't in the first two columns.
function table(headers, rows, opts) {
  const keyColsAttr = opts && opts.keyCols ? ` data-key-cols="${Number(opts.keyCols)}"` : '';
  const ths = headers.map((h) => `<th>${esc(h)}</th>`).join('');
  const trs = rows.map((r) =>
    `<tr>${r.map((c, i) => `<td data-label="${esc(headers[i] || '')}">${c == null ? '' : c}</td>`).join('')}</tr>`
  ).join('');
  return `<table class="data-table"${keyColsAttr}><thead><tr>${ths}</tr></thead><tbody>${trs}</tbody></table>`;
}
