...
function csrfValid(req) {
  ...
  // Masked form — what every page renders now.
  if (submitted.length === MASKED_HEX) {
    const raw = unmaskToken(submitted);
    return raw !== null && sameToken(raw, session);
  }
  // Unmasked form — a page rendered before this deploy whose form is still
  // open in someone's browser. Accepted so the rollout does not turn every
  // in-flight submission into a security-check failure. Safe to delete once
  // no pre-deploy sessions remain.
  if (submitted.length === HEX) return sameToken(submitted, session);
  return false;
}
