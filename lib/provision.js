...
async function signupChurch({ churchName, name, email, password }) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await tx.specialCategory.createMany({
      data: DEFAULT_SPECIAL_CATEGORIES.map(([categoryName, description]) => ({
        churchId: church.id,
        categoryName,
        description,
      })),
    ...
    await tx.serviceType.createMany({
      data: DEFAULT_SERVICE_TYPES.map((typeName) => ({ churchId: church.id, typeName })),
    });
    await tx.expenseCategory.createMany({
      data: DEFAULT_EXPENSE_CATEGORIES.map(([categoryName, description]) => ({ churchId: church.id, categoryName, description })),
    });
    return { church, user };
